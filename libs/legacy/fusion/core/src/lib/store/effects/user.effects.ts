...
export class AuthEffects {
      ...
      this.actions.pipe(
        ...
        tap((data: LoginSuccess) => {
          ...
          localStorage.setItem(
            MetaConstants.USER_LOCAL_STORAGE_KEY,
            ...
            if (
              ...
              Logger.trace(
                `User Effect  => LoginSuccess effect => Bypass Passowrd strength check for Delegator and SSO user => User Info: ${JSON.stringify(
                  data?.payload?.user
                )} | current Token: ${JSON.stringify(token)} | active Route:${
                  this.route?.url
                }`
              );
  ...
  LoginWithRefreshToken: Observable<any> = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap((payload: any) => {
        ...
        return this.authService.login(newPayload).pipe(
          map((response: TokenModel) => {
            this.userInputModel = newPayload;
            const data: { tokenData; userData; tokenSessionInfo } =
              this.handleTokenExpiration(response);
  ...
  LoginWithRefreshTokenSuccess: Observable<any> = createEffect(
    () =>
      this.actions.pipe(
  ...
  AutoRenewToken: Observable<any> = createEffect(() =>
    this.actions.pipe(
  ...
  public SetFacilityWithStatuses: Observable<any> = createEffect(
    () =>
      this.actions.pipe(
  ...
  public UpdateFacilityWiseStatuses: Observable<any> = createEffect(
    () =>
      this.actions.pipe(
        ofType(OrgActionTypes.UpdateFacilityWiseStatuses),
        tap((data: UpdateFacilityWiseStatuses) => {
          sessionStorage.setItem('FacilityId', data.payload.FacilityId);
          sessionStorage.setItem(
            'StatusCount',
            JSON.stringify(data.payload.StatusCount)
  ...
  public DelegatorLogout: Observable<any> = createEffect(
    () =>
      this.actions.pipe(
