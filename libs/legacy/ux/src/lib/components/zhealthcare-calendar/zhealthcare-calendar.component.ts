...
import {
  AfterViewInit, Attribute, ChangeDetectorRef, Component, DoCheck, ElementRef, EventEmitter, HostBinding, HostListener, Input, OnDestroy, Optional, Output, Self
} from '@angular/core'
import {
  AbstractControl, ControlValueAccessor, FormGroupDirective, NgControl,
  NgForm, ValidationErrors, Validator, ValidatorFn,
  Validators
} from '@angular/forms'
import {
  CanDisable, CanUpdateErrorState, DateAdapter, ErrorStateMatcher, HasTabIndex, mixinDisabled,
  mixinErrorState, mixinTabIndex, ThemePalette
} from '@angular/material/core'
import {
  MatCalendar, MatCalendarCellClassFunction, MatDatepicker,
 ...
 */
@Component({
  selector: 'zhealthcare-calendar',
  templateUrl: './zhealthcare-calendar.component.html',
  styleUrls: [ './zhealthcare-calendar.component.scss' ],
  providers: [
    { provide: MatFormFieldControl, useExisting: zhealthcareCalendarComponent }
  ]
})

export class zhealthcareCalendarComponent<D = Date>
  ...
  @Input()
  @HostBinding()
  public id = `zhealthcare-calendar-${zhealthcareCalendarComponent.nextId++}`;
  ...
  /** Emits when a change event is fired on this `zhealthcare-calendar` element. */
  ...
  /** Whether `zhealthcare-calendar` element has focus. */
  public focused = false;
  /** A name for this control that can be used by mat-form-field. */
  public controlType? = 'zhealthcare-calendar';
  /**
  ...
  /**
   * The datepicker (or calendar - for inline view) that this `zhealthcare-calendar` element is
   * associated with.
   */
  ...
  /** The value of the `zhealthcare-calendar` control. */
  ...
  public set matDatepicker(value: MatDatepicker<D> | MatCalendar<D>) {
    if (!value || (!(value instanceof MatDatepicker) && !(value instanceof MatCalendar))) {
      throw new TypeError(
        `Either "matDatepicker" or "matCalendar" attribute of "zhealthcare-calendar" is required and
  ...
  /** Focuses the `zhealthcare-calendar` element. */
  @HostListener('focus')
  public focus(): void {
    if (!this.disabled) {
      this.focused = true;
  ...
  /** Used to leave focus from the `zhealthcare-calendar` element. */
  @HostListener('blur')
  public blur(): void {
    this.focused = false;
    if (!this.disabled) {
