...
export class NgxTimepickerTimeControlComponent implements OnChanges {
  ...
  increase(): void {
    if (!this.disabled) {
      ...
      if (this.isSelectedTimeDisabled(nextTime)) {
        nextTime = this.getAvailableTime(
          nextTime,
          this.getNextAvailableTime.bind(this)
        );
      }
  ...
  decrease(): void {
    if (!this.disabled) {
      ...
      if (this.isSelectedTimeDisabled(previousTime)) {
        previousTime = this.getAvailableTime(
          previousTime,
          this.getPrevAvailableTime.bind(this)
        );
      }
  ...
  private isSelectedTimeDisabled(time: number): boolean {
    return this.timeList.find(
      (faceTime: ClockFaceTime) => faceTime.time === time
    ).disabled;
  }
