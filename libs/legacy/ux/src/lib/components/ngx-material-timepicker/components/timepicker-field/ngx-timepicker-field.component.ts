  ...
  implements OnInit, OnDestroy, ControlValueAccessor
{
  ...
  set format(value: number) {
    ...
    const isDynamicallyChanged =
      value && this.previousFormat && this.previousFormat !== this._format;
  ...
  ngOnInit() {
    ...
    if (this.format === 12) {
      this.timepickerService.selectedPeriod
        .pipe(
          distinctUntilChanged<TimePeriod>(),
          tap((period: TimePeriod) => (this.period = period)),
          tap(
            (period) =>
              (this.isChangePeriodDisabled = this.isPeriodDisabled(period))
          ),
  ...
  private updateAvailableMinutes(): void {
    this.minutesList = TimepickerTimeUtils.disableMinutes(
      this.minutesList,
      this.selectedHour,
      {
        min: this.min as DateTime,
  ...
  private initTime(time): void {
    const isDefaultTimeAvailable = TimeAdapter.isTimeAvailable(
      time,
      this.min as DateTime,
      this.max as DateTime,
      'minutes',
    ...
    if (!isDefaultTimeAvailable) {
      if (this.min) {
        this.updateTime(
          TimeAdapter.fromDateTimeToString(this.min as DateTime, this.format)
        );
        return;
      }
      if (this.max) {
        this.updateTime(
          TimeAdapter.fromDateTimeToString(this.max as DateTime, this.format)
        );
        return;
      }
    }
