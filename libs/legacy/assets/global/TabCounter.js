var TabCount = /** @class */ (function () {
  function TabCount() {
    ...
    this.clearList = function (data) {
      ...
      listIds.forEach(function (id) {
        if (
          data.list[id].lastActive +
            Math.max(8000, _this.updateInterval * 1.5) <
          now
        ) {
          //If tab last update is older get rid of it.
          delete data.list[id];
        }
      });
