...
import { Button, ResourceLoader, ImageResourceLoader, IMAGEVIEWER_CONFIG, ImageViewerConfig, toSquareAngle, IMAGEVIEWER_CONFIG_DEFAULT } from '../../models';
...
@Component({
  ...
  template: `
    <canvas style="cursor:grab; border-radius:8px ; border:1px solid #B3B3B3; width:100%" #imageContainer [height]="height"
...
export class ImageViewerComponent implements AfterViewInit, OnDestroy {
  ...
  private readonly _listenDestroyList = [];
  ...
  constructor(
    ...
    private readonly _imageCache: ImageCacheService,
    @Inject(IMAGEVIEWER_CONFIG) private readonly config: ImageViewerConfig
  ) {
    ...
    this._downloadButton = new Button(this.config.downloadButton, this.config.buttonStyle);
  ...
  ngAfterViewInit() {
    ...
    { this.zoomIn();
       return false; };
    this._downloadButton.onClick = (evt) =>
     { this.download();
       return false; };
    this._rotateLeftButton.onClick = (evt) =>
     { this.rotateLeft();
      return false; };
    this._rotateRightButton.onClick = (evt) => { this.rotateRight();
       return false; };
    this._resetButton.onClick = (evt) => { this.resetImage();
      return false; };
  ...
  setUpResource() {
    if ((!this._resource || !(this._resource instanceof ImageResourceLoader))) {
  ...
  processTouchEvent(evt) {
    ...
    if (!this._touchStartState.viewport)
     { this._touchStartState.viewport = Object.assign({},
      this._resource.viewport); }
  ...
  private download() {
    this.downloadUrl(this._src, 'image.png');
    URL.revokeObjectURL(this._blobimage);

  }

  private downloadUrl(url: string | File, fileName: string) {
  ...
  private drawButtons(ctx) {
    ...
    const labelWidth = 50;
    const x = radius + padding + labelWidth; // PrevPageButton
  ...
  private extendsDefaultConfig(cfg: ImageViewerConfig) {
    ...
    { localCfg.downloadButton = Object.assign(defaultCfg.downloadButton, cfg.downloadButton); }
    return localCfg;
  }
