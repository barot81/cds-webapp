...
export class Button {
  ...
  constructor(config: ButtonConfig, private style: ButtonStyle) {
  ...
  draw(ctx, x, y, radius) {
    ...
    // drawing settings
    const isHover =
      typeof this.hover === 'function' ? this.hover() : this.hover;
...
export abstract class ResourceLoader {
  ...
  public resetViewport(canvasDim: Dimension): boolean {
    ...
    if (
      (canvas.height / this._image.height) * this._image.width <=
      canvas.width
    ) {
...
export function toSquareAngle(angle: number) {
  return (
    90 * ((Math.trunc(angle / 90) + (Math.trunc(angle % 90) > 45 ? 1 : 0)) % 4)
  );
}
