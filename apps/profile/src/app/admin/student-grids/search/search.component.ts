...
import { CampusLookup, CategoryLookup, EnrollmentTermLookup } from '@zhealthcare/cohort/models';
import { CohortFacade } from '@zhealthcare/cohort/store';
import {
  FusionConfigService, Logger, MethodType,
  UserFacade
} from '@zhealthcare/fusion/core';
import { ExcelModel } from '@zhealthcare/fusion/models';
import {
  ExportExcelService, FeatureMetadataService, FusionNavigationService
} from '@zhealthcare/fusion/services';
import {
  ColumnOption, DataSourceComponentService, DataSourceFacade, DataSourceResponse, DataSourceService, Filter,
  FusionDataSource, Item,
  Sort
} from '@zhealthcare/plugin/data-source';
import { FileConfiguration, FileEndpoint } from '@zhealthcare/plugin/file-upload';
import {
  ProfileDataSource
} from '@zhealthcare/profile/constants';
import {
  EnrollmentCalendar,
  FEATURECODE,
  GraduationCalendar,
  GraduationTermLookup,
  SearchProfile,
  ShowMore,
  StudentSerachColInfo, TagMaster
} from '@zhealthcare/profile/models';
import {
  BulkUpdateApiClient,
  LookupSandbox,
  ProfileAdminLayoutService,
  ProfileApiClient
} from '@zhealthcare/profile/services';
import { ProfileFormsService } from '@zhealthcare/profile/shared';
import { ProfileListFacade, TagMasterFacade } from '@zhealthcare/profile/store';
...
import {
  ...
  distinctUntilChanged, map, takeUntil, tap
} from 'rxjs/operators';


@Component({
  selector: 'profile-admin-search',
...
export class SearchComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    public _profileFormsService: ProfileFormsService,
    private readonly _profileListFacade: ProfileListFacade,
    ...
    private readonly _bulkUpdateClient: BulkUpdateApiClient
  ) {
    ...
    this.columnInformation = StudentSerachColInfo;
    ...
    this._profileAPIClient.onStudentAdded.subscribe((isAdded) => {
      if (isAdded) {
        this.resetFilter();
    ...
    this._profileAPIClient.onFilterChange.subscribe((change) => {
      if (change) {
        this.initPagination();
      }
    });
    ...
    this.datasourceFacade.fusionDataSource$
      ...
      .subscribe((res) => {
        if (res) {
          this.gridData = res;
          this._profileListFacade.profileListLoaded({ result: res?.data, count: res?.total });
        }
      });
  ...
  ngOnInit(): void {
    ...
    //setup check
    if (
      this._fusionNavigationService.getNavigationItem('complianceRequirements')
    ) {
      this.hasPublishAccess = true;
    }
  ...
  sortByCreatedTimeStamp() {
    ...
    this._profileAPIClient.onStudentAdded.next(false);
    this.initPagination();
  }
    ...
    this.datasourceFacade.fusionDataSource$
      ...
      .subscribe((x) => {
        ...
        sessionStorage.setItem(
          ProfileDataSource.Student_Grid_Datasource,
          JSON.stringify(x)
        );
        this.setFilterValue();
      });
  ...
  setDefaultFilter() {
    if (sessionStorage.getItem(ProfileDataSource.Student_Grid_Datasource)) {
      if (localStorage.getItem('fusionDataSource')) {
        this.resetFilter();
      }
      const filterSessionData = JSON.parse(
        sessionStorage.getItem(ProfileDataSource.Student_Grid_Datasource)
      );
  ...
  clearSelectionOnBulkUpdateComplete() {
    this._bulkUpdateClient.bulkUpdateCompletionStatus$.pipe(takeUntil(this._unsubscribe))
